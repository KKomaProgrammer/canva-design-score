...
기본 모델은 비용 효율적인 `gpt-5.6-luna`이며, 더 엄격한 평가가 필요할 때 `gpt-5.6-terra`를 선택할 수 있습니다. 확장 프로그램은 모든 `.JFv1rQ` 요소를 DOM 순서대로 찾고, 각 요소 안의 첫 번째 `img`를 PNG로 변환합니다. PNG 폭은 전체 페이지 수에 따라 512~1280px 범위에서 자동 조정됩니다.

분석이 끝나면 결과 전용 상호작용 SVG를 다운로드할 수 있습니다. 우측 상단 기록 버튼에서는 저장된 상세 분석을 다시 열거나 삭제할 수 있습니다. 삭제한 기록은 UI에서 영구 삭제하지 않고 `chrome.storage.local`의 `analysisTrash`로 이동하며, 저장 한도를 넘긴 가장 오래된 기록부터 자동 정리됩니다.
